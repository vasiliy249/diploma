...
Запуск гостевой ОС включает выполнение гостевого кода, обработки таймеров, выполнение операций ввода/вывода и реакции на комманды системы мониторинга QEMU. Чтобы обрабатывать все эти задачи разом требуется архитектура, способная реагировать на внешние «сигналы» программно-аппаратных ресурсов, не останавливая при этом работу гостевой ОС даже в случае длительных операций (например, дисковых операция ввода/вывода или мониторинговых команд).

Есть два популярных подхода, на базе которых разработаны программные архитектуры, удовлетворяющие нашим условиям:
...
QEMU, на самом деле, сочетает в себе событийную потоковую модели одновременно. Это разумно, поскольку цикл событий не может воспользоваться преимуществами многоядерности или многопроцессорности, так как имеет только один поток исполнения. Кроме того, иногда проще выделить отдельный поток, чтобы выполнить одну частную задачу, а не интегрировать её в глобальный событийный цикл. Тем не менее, ядро QEMU является событийно-управляемым и большинство кода выполняется синхронно.

Основной цикл событий QEMU имплементирован в main_loop_wait(), он выполняет следующие задачи:

* Ожидает, чтобы стали доступными для чтения/записи файловые дискрипторы. Нахождение этой функциональности в ядре QEMU объясняется тем, что файлы играют центральную роль в доступе к ресурсам, т. к. обычные файлы, директории, сокеты, именованные каналы и т. п. для любого прикладного приложения - суть файловые дискрипторы.
