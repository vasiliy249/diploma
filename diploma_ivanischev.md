...
Когда виртуальная машина выключается, процесс QEMU завершается, но перезагрузка гостевой ОС выполняется без перезапуска процесса QEMU.
...
Когда дескриптор файла становится доступным, таймер экспирируется или запускается отложенный обработчик прерываний, цикл событий вызывает (синхронный) функцию-обработчик события. Каждая такая функция-обработчик подчиняется двум простым правилам:
...
2. Не делаются никакие блокирущие системные вызовы или иные долгие операции. Нарушение этого правила влечёт к остановкам гостевой ОС в следствие остановки выполнения цикла из-за задержек в обработчиках событий.
