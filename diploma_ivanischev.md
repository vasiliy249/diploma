...
Каждый хайв устроен таким образом, что данные внутри него хранятся в ячейках (на англ. cells). В памяти операционной системы (и физической памяти, соответственно) реестр редко хранится в линейно-равномерном виде, чаще всего его структура сильно фрагментирована. Поэтому для того, чтобы получить полную информацию о реестре необходимо осуществить «трансляцию» адресов ячеек [8].
...
После того, как запускается QEMU-процесс, происходит выделение и резервирование фиксированного блока виртуальной памяти (адресной арены) хост-процесса под оперативную память виртуальной машины, т.е. «физической» памяти гостевой ОС (см. рис. 3.2.).
...
Запуск гостевой ОС включает выполнение гостевого кода, обработки таймеров, выполнение операций ввода/вывода и реакции на комманды системы мониторинга QEMU. Делая все эти вещи сразу требуется архитектура, способная опосредовать ресурсы безопасным способом, не останавливая работу гостевой ОС в случае, когда диск ввода/вывода или монитор команд требуют длительного времени для выполнения. Есть две популярных архитектуры программ, которые должны реагировать на события из нескольких источников:

* Параллельная архитектура разделяет работу по процессам или потокам, которые могут выполняться одновременно. Эта архитектура будет называться «потоковая архитектура»

* Событийная архитектура реагирует на события путём выполнения главного цикла, что направляет в обработчики событий. Это обычно осуществляется с помощью системных вызовов select(2) или poll(2) для ожидания нескольких файловых дискрипторов

QEMU на самом деле сочетает в себе событийно-управляемое программирование с потоками. Это имеет смысл потому что цикл событий не может воспользоваться несколькими ядрами, так как он имеет только один поток исполнения. Кроме того, иногда проще написать специальный поток, чтобы разгрузить одну конкретную задачу, а не интегрировать его в архитектуру управляемых событий. Тем не менее, ядро QEMU является событийно-управляемым и большинство кода выполняется в этой среде.

Событийно-управляемая архитектура сосредоточена вокруг цикла событий, который отправляет события в функции-обработчики. Основной цикл событий QEMU это main_loop_wait() , он выполняет следующие задачи:

* Ожидает файловые дискрипторы, чтобы стать возможным для чтения/записи. Файловые дискрипторы играют решающую роль, потому что файлы, сокеты, ???, и другие различные ресурсы являются файловыми дискрипторами. Файловые дискрипторы могут быть добавлены с помощью qemu_set_fd_handler().

* Запускает ??? таймеры. Таймеры могут быть добавлены с помощью qemu_mod_timer().

* Запускает bottom-halves, которые, как таймеры, истекают сразу???. BHs используются чтобы избежать повторного входа и переполнения стека вызовов. Они могут быть добавлены с помощью qemu_bh_schedule ().

Когда дескриптор файла будет готов, таймеры и BH планируются, цикл событий вызывает функцию обратного вызова, которая реагирует на события. Обратная связь имеет два простых правила относительно их окружения:

* Ни один другой код ядра не будет выполнятся поскольку в синхронизации нет необходимости. Обратные вызовы выполняются последовательно и атомарно по отношению к другому коду ядра. Существует только один поток управления, который выполняет код ядра в любой момент времени

* Не блокируемые системные вызовы или долго выполняющиеся вычисления должны быть выполнены??? Так цикл событий ждёт обратного вызова для того, чтобы вернуться прежде чем продолжить работу с другими событиями, важно не тратить неограниченное количество времени на функции обратного вызова. Нарушение этого правила влечёт к остановке гостевой ОС и монитор становится неспособным отвечать на запросы???

Второе правило иногда трудно соблюдать и есть код QEMU который это блокирует???. На самом деле есть ещё цикл, вложенный в qemu_aio_wait(), который ждёт подмножество событий, которые обрабатывает цикл событий верхнего уровня. Надеемся, эти нарушения будут исправлены в будущей реструктуризации кода. Новый код практически никогда не имеет законных оснований, чтобы использовать блокировку, и одно из решений заключается в использовании специализированных рабочих потоков чтобы разгрузить длительное выполнение или блокировку кода.
...
Для начала следует упомянуть, что управлять виртуальными машинами, используя команды QEMU напрямую представляется нерациональным ввиду: (а) перспективы использования в качестве бекенд-гипервизора не только QEMU/KVM; (б) автоматизации менеджмента виртуальными окружениями; (в) унификации бизнес-логики анализа и мониторинга. Поэтому было решено применить библиотеку управления виртуализацией – libvirt, т. к. на данный момент она поддерживает большинство известных гипервизоров и унифицирует управление ими через обобщенный API. Благодаря этому, программный комплекс становится универсальным и независящим от конкретного гипервизора.
...
На первом этапе разработки архитектуры программного комплекса был реализован доступ к физической памяти гостевых машин последовательно, через libvirt и QEMU/KVM. Разумеется, хотелось бы получить доступ к памяти напрямую с хост-платформы, не используя никаких «прослоек». С одной стороны, мы могли бы сделать форк (на англ. fork - системный вызов, создающий новый процесс, который является практически полной копией процесса-родителя) QEMU процесса и работать с ним. Но ввиду сегодняшней имплементации QEMU, а именно многопоточности процессов, мы не можем использовать данный подход, поскольку процесс-потомок, получившийся в результате форка, может быть «мёртвым»???. Поэтому самый простой способ решить данную проблему – это воспользоваться командами QEMU – что и было реализовано на первом шаге разработки программного комплекса.

Поэтому в дальнейшем планируется проанализировать устройство отображения памяти гостевых ОС на память сервера виртуализации, что позволит нам делать одномоментные снимки памяти напрямую с хост-платформы.
