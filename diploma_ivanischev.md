...
На первом этапе разработки архитектуры программного комплекса доступ к физической памяти гостевых машин QEMU/KVM был реализован через libvirt. Разумеется, хотелось бы получить доступ к памяти напрямую с хост-платформы, не используя при этом никаких промежуточных элементов, но пока это видится недостижимым.
...
Тем не менее, не смотря на описанные сложности, в дальнейшем планируется проанализировать устройство отображения памяти гостевых ОС на память сервера виртуализации более подробно с целью поиска надежных средств для атомарных снимков памяти напрямую с хост-платформы (см. рис. 3.7).
